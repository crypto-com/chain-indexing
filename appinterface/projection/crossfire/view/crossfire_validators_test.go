...
var _ = Describe("Blocks", func() {
	WithTestPgxConn(func(conn *pg.PgxConn, migrate *pg.Migrate) {
		...
		Describe("Insert", func() {
			It("should insert validator into view", func() {
				var err error
				...
				crossfireValidatorView := view.NewCrossfireValidators(conn.ToHandle())

				err = crossfireValidatorView.Upsert(&validator)
				Expect(err).To(BeNil())

				var actual *view.CrossfireValidatorRow
				actual, err = crossfireValidatorView.FindBy(view.CrossfireValidatorIdentity{
