...
var _ = Describe("Crossfire", func() {
	WithTestPgxConn(func(pgConn *pg.PgxConn, pgMigrate *pg.Migrate) {
		...
		phaseOneEmptyBlockCreatedEvent := event_usecase.NewBlockCreated(&usecase_model.Block{
			Height:          int64(500),
		...
		It("should correctly compute phase 1 and phase 2 commitment rank", func() {
			...
			err := projection.HandleEvents(phase1Height, []event_entity.Event{
				phaseOneEmptyBlockCreatedEvent,
				validator1CreatedEvent,
			...
			Expect(err).To(BeNil())
			Expect(phase1BlockCount).To(Equal(int64(2)))
