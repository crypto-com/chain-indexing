...
// behaves the same except it will panic on any error.
func MustNewCoinFromString(denom string, amountStr string) *Coin {
	amount, err := NewCoinFromString(denom, amountStr)
	if err != nil {
		panic(err)
	}
	return amount
...
// UnmarshalJSON implements a custom JSON unmarshaller for the Coins type to allow
// empty array to be decoded as nil Coins.
// Added to match the behaviour of Cosmos marshaller
func (coins *Coins) UnmarshalJSON(data []byte) error {
	if string(data) == "[]" {
		return nil
	}

	var result coinsJSON
	if err := json.Unmarshal(data, &result); err != nil {
		return err
	}
	*coins = make(Coins, 0, len(result))
...
func (coins *Coins) Validate() error {
	...
	case 1:
		...
		if !(*coins)[0].IsPositive() {
			return fmt.Errorf("coin %s amount is not positive", (*coins)[0].String())
		}
		return nil
...
// MustParseCoinNormalized parses and normalize a cli input for one coin type.
// It behaves the same as ParseCoinNormalized except it panics on any error.
func MustParseCoinNormalized(coinStr string) Coin {
	coin, err := ParseCoinNormalized(coinStr)
	if err != nil {
		panic(err)
	}
...
// MustParseCoinsNormalized will parse out a list of coins separated by commas
// behaves the same as ParseCoinsNormalized except it panics on any error.
func MustParseCoinsNormalized(coinStr string) Coins {
	coins, err := ParseCoinsNormalized(coinStr)
	if err != nil {
		panic(err)
	}
