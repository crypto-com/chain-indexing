...
var _ = Describe("ParseMsgCommands", func() {
	Describe("MsgTierUndelegate", func() {
		It("should parse tieredrewards.MsgTierUndelegate command in the transaction", func() {
			...
			tx := MustParseTxsResp(usecase_parser_test.TX_MSG_TIER_UNDELEGATE_TXS_RESP)
			...
			Expect(cmds).To(Equal([]command.Command{
				command_usecase.NewCreateMsgTierUndelegate(
					...
					model.MsgTierUndelegateParams{
						RawMsgTierUndelegate: model.RawMsgTierUndelegate{
							Type:       "/chainmain.tieredrewards.v1.MsgTierUndelegate",
