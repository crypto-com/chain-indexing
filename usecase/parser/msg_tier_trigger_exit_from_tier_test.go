...
var _ = Describe("ParseMsgCommands", func() {
	Describe("MsgTierTriggerExitFromTier", func() {
		It("should parse tieredrewards.MsgTriggerExitFromTier command in the transaction", func() {
			...
			tx := MustParseTxsResp(usecase_parser_test.TX_MSG_TIER_TRIGGER_EXIT_FROM_TIER_TXS_RESP)
			...
			Expect(cmds).To(Equal([]command.Command{
				command_usecase.NewCreateMsgTierTriggerExitFromTier(
					...
					model.MsgTriggerExitFromTierParams{
						RawMsgTriggerExitFromTier: model.RawMsgTriggerExitFromTier{
							Type:       "/chainmain.tieredrewards.v1.MsgTriggerExitFromTier",
