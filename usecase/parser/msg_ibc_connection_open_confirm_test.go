...
var _ = Describe("ParseMsgCommands", func() {
	Describe("MsgIBCConnectionOpenConfirm", func() {
		It("should parse Msg commands when there is MsgIBCConnectionOpenAck in the transaction", func() {
			expected := `{
  "name": "MsgConnectionOpenConfirmCreated",
  "version": 1,
  "height": 13,
  "uuid": "{UUID}",
  ...
  "params": {
    ...
    "counterpartyConnectionId": "connection-0"
			...
			cmd := cmds[1]
			Expect(cmd.Name()).To(Equal("CreateMsgIBCConnectionOpenConfirm"))
			...
			Expect(possibleSignerAddress[1]).To(Equal("cro1dulwqgcdpemn8c34sjd92fxepz5p0sqpeevw7f"))
		})
	})
})
