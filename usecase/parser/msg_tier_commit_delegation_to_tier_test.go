...
var _ = Describe("ParseMsgCommands", func() {
	Describe("MsgTierCommitDelegationToTier", func() {
		It("should parse tieredrewards.MsgCommitDelegationToTier command in the transaction", func() {
			...
			tx := MustParseTxsResp(usecase_parser_test.TX_MSG_TIER_COMMIT_DELEGATION_TXS_RESP)
			...
			Expect(cmds).To(Equal([]command.Command{
				command_usecase.NewCreateMsgTierCommitDelegationToTier(
					...
					model.MsgCommitDelegationToTierParams{
						RawMsgCommitDelegationToTier: model.RawMsgCommitDelegationToTier{
							Type:                   "/chainmain.tieredrewards.v1.MsgCommitDelegationToTier",
							DelegatorAddress:       "cro1dulwqgcdpemn8c34sjd92fxepz5p0sqpeevw84",
							ValidatorAddress:       "crocncl1kkqxv3szgh099xezt7y38t5anqzue4s3fhp2tm",
							Amount:                 "50000basecro",
							Id:                     2,
							TriggerExitImmediately: false,
						},
					},
				),
			}))
