...
`
			...
			block, _, _ := tendermint.ParseBlockResp(strings.NewReader(
				usecase_parser_test.TX_CHAINMAIN_MSG_SUBMIT_TX_BLOCK_RESP,
			))
			blockResults, _ := tendermint.ParseBlockResultsResp(strings.NewReader(
				usecase_parser_test.TX_CHAINMAIN_MSG_SUBMIT_TX_BLOCK_RESULTS_RESP,
			), &tendermint.Base64BlockResultEventAttributeDecoder{})

			tx := MustParseTxsResp(usecase_parser_test.TX_CHAINMAIN_MSG_SUBMIT_TX_TXS_RESP)
			...
			typedEvent := untypedEvent.(*event.ChainmainMsgSubmitTx)
