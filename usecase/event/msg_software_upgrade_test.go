...
var _ = Describe("Event", func() {
	Describe("En/DecodeMsgSoftwareUpgrade", func() {
		...
		It("should able to encode and decode to the same event", func() {
			...
			anyPlan := v1_model.MsgSoftwareUpgradePlan{
				Name:   "any name",
			...
			anyParams := v1_model.MsgSoftwareUpgradeParams{
			...
			event := event_usecase.NewMsgSoftwareUpgrade(event_usecase.MsgCommonParams{
			...
			decodedEvent, err := registry.DecodeByType(
				event_usecase.MSG_SOFTWARE_UPGRADE_CREATED, 1, []byte(encoded),
			)
			Expect(err).To(BeNil())
			Expect(decodedEvent).To(Equal(event))
			typedEvent, _ := decodedEvent.(*event_usecase.MsgSoftwareUpgrade)
			Expect(typedEvent.Name()).To(Equal(event_usecase.MSG_SOFTWARE_UPGRADE_CREATED))
			...
			Expect(typedEvent.Plan).To(Equal(anyPlan))
		...
		It("should able to encode and decode to failed event", func() {
			...
			decodedEvent, err := registry.DecodeByType(
				event_usecase.MSG_SOFTWARE_UPGRADE_FAILED, 1, []byte(encoded),
			)
			Expect(err).To(BeNil())
			Expect(decodedEvent).To(Equal(event))
			typedEvent, _ := decodedEvent.(*event_usecase.MsgSoftwareUpgrade)
			Expect(typedEvent.Name()).To(Equal(event_usecase.MSG_SOFTWARE_UPGRADE_FAILED))
			Expect(typedEvent.Version()).To(Equal(1))
