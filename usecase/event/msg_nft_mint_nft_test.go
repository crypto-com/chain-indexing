...
var _ = Describe("Event", func() {
	...
	Describe("En/DecodeMsgNFTMintNFT", func() {
		It("should able to encode and decode to the same event", func() {
			...
			anyRecipient := "cro1nk4rq3q46ltgjghxz80hy385p9uj0tf58apkcd"
			...
			decodedEvent, err := registry.DecodeByType(
				event_usecase.MSG_NFT_MINT_NFT_CREATED, 1, []byte(encoded),
			)
			Expect(err).To(BeNil())
			Expect(decodedEvent).To(Equal(event))
			typedEvent, _ := decodedEvent.(*event_usecase.MsgNFTMintNFT)
			Expect(typedEvent.Name()).To(Equal(event_usecase.MSG_NFT_MINT_NFT_CREATED))
