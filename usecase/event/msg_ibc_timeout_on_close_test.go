...
var _ = Describe("Event", func() {
	...
	Describe("En/DecodeMsgIBCTimeoutOnClose", func() {
		It("should able to encode and decode to the same event", func() {
			...
			var anyRawMsg ibc_model.RawMsgTimeoutOnClose
			json2.MustUnmarshalFromString(`
{
  ...
  "signer": "cro1q040rm026jmpfmxdsj6q9phm9tdceepnsau6me"
			...
			anyParams := ibc_model.MsgTimeoutOnCloseParams{
				RawMsgTimeoutOnClose: anyRawMsg,
			...
			event := event_usecase.NewMsgIBCTimeoutOnClose(event_usecase.MsgCommonParams{
			...
			decodedEvent, err := registry.DecodeByType(
				event_usecase.MSG_IBC_TIMEOUT_ON_CLOSE_CREATED, 1, []byte(encoded),
			)
			Expect(err).To(BeNil())
			Expect(decodedEvent).To(Equal(event))
			typedEvent, _ := decodedEvent.(*event_usecase.MsgIBCTimeoutOnClose)
			Expect(typedEvent.Name()).To(Equal(event_usecase.MSG_IBC_TIMEOUT_ON_CLOSE_CREATED))
			...
			Expect(typedEvent.Params.RawMsgTimeoutOnClose).To(Equal(anyParams.RawMsgTimeoutOnClose))
			...
			json2.MustUnmarshalFromString(`
{
  ...
  "signer": "cro1q040rm026jmpfmxdsj6q9phm9tdceepnsau6me"
			...
			decodedEvent, err := registry.DecodeByType(
				event_usecase.MSG_IBC_TIMEOUT_ON_CLOSE_FAILED, 1, []byte(encoded),
			)
			Expect(err).To(BeNil())
			Expect(decodedEvent).To(Equal(event))
			typedEvent, _ := decodedEvent.(*event_usecase.MsgIBCTimeoutOnClose)
			Expect(typedEvent.Name()).To(Equal(event_usecase.MSG_IBC_TIMEOUT_ON_CLOSE_FAILED))
			Expect(typedEvent.Version()).To(Equal(1))
