...
var _ = Describe("Validator projection", func() {
	WithProjectionTestEnv(func(testEnv ProjectionTestEnv) {
		...
		It("should flag the validator staut to attention after two MsgEditValidator event", func() {
			validatorView := validator_view.NewValidators(pgxConn.ToHandle())
			...
			// 	MaxChangeRate: "0.010000000000000000",
			// }
			maybeCommissionRate := "0.100000000000000000"
			maybeMinSelfDelegation := "1"

			editValidatorParams := usecase_model.MsgEditValidatorParams{
				Description:            description,
