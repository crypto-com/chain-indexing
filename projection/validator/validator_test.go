	...
	testCases := []struct {
	...
	}{
		{
			...
			MockFunc: func(events []event_entity.Event) (mocks []*testify_mock.Mock) {
				...
				MaybeOperatorAddress := "tcrocncl1fmprm0sjy6lz9llv7rltn0v2azzwcwzvr4ufus"
				fmt.Println("===> s1", MaybeOperatorAddress, &MaybeOperatorAddress)

				mockValidatorsView.On(
					"FindBy",
					validator_view.ValidatorIdentity{
						MaybeOperatorAddress: &MaybeOperatorAddress,
					},
				).Return(validator_view.ValidatorRow{
