...
func TestBridgePendingActivity_HandleEvents(t *testing.T) {
	testCases := []struct {
		...
		{
			Name: "It should not handle IBC transfer from non-listening source channel",
			Config: bridge_pending_activity.Config{
				ThisChainId:           "chain-0",
				ThisChainName:         "this",
				CounterpartyChainName: "counterparty",
				ChannelId:             "channel-0",
				StartingHeight:        0,
			},
			Events: []entity_event.Event{
		...
		},
		{
			...
			MockFunc: func() (mocks []*testify_mock.Mock, assertFunc func()) {
				...
				mockBridgePendingActivitiesView.On("Insert", &view.BridgePendingActivityInsertRow{
					...
					MaybeTransactionId:            primptr.String("TxHash"),
					BridgeType:                    types.BRIDGE_TYPE_IBC,
					LinkId:                        "source:this-transactionId:TxHash-failedOnChain",
					Direction:                     types.DIRECTION_OUTGOING,
					FromChainId:                   "this",
					MaybeFromAddress:              primptr.String("from"),
					MaybeFromSmartContractAddress: nil,
		...
		{
			Name: "It should not handle MsgRecvPacket event from non-listening destination channel",
			Config: bridge_pending_activity.Config{
				ThisChainId:           "chain-0",
				ThisChainName:         "this",
				CounterpartyChainName: "counterparty",
		...
		},
		{
			...
			MockFunc: func() (mocks []*testify_mock.Mock, assertFunc func()) {
				...
				mockBridgePendingActivitiesView.On("Insert", &view.BridgePendingActivityInsertRow{
					...
					MaybeTransactionId:            primptr.String("TxHash"),
					BridgeType:                    types.BRIDGE_TYPE_IBC,
					LinkId:                        "source:counterparty-sequence:0",
		...
		{
			Name: "It should not handle MsgAcknowledgement event from non-listening source channel",
			Config: bridge_pending_activity.Config{
				ThisChainId:           "chain-0",
				ThisChainName:         "this",
				CounterpartyChainName: "counterparty",
