...
func (messagesView *MockMessagesView) FindById(denomId string, tokenId string) (*MessageRow, error) {
	mockArgs := messagesView.Called(denomId, tokenId)
	row, _ := mockArgs.Get(0).(*MessageRow)
...
func (messagesView *MockMessagesView) Update(messageRow MessageRow) error {
	mockArgs := messagesView.Called(messageRow)
	return mockArgs.Error(0)
}
