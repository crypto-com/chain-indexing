...
var _ = Describe("PgxTypeConv", func() {
	var pgxTypeConv infrastructure.PgxTypeConv

	BeforeEach(func() {
		//nolint:staticcheck
		pgxTypeConv = infrastructure.PgxTypeConv{}
	})
	...
	Describe("Tton", func() {
		...
		It("should convert time to native time type", func() {
			...
			maybeAnyTime := primptr.Time(time.Unix(707810766, 0))
	...
	Describe("NtotReader", func() {
		...
		It("should return time.Time of the time value", func() {
			expected := time.Unix(707810766, 0)
