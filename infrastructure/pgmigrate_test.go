...
func MustNewTestPgConn(config *PgConnConfig) *TestPgConn {
...
var _ = Describe("PgMigrate", func() {
	WithTestPgConnConfig(func(config *PgConnConfig) {
		var pgConn *TestPgConn
		BeforeEach(func() {
			migrate, _ := NewPgMigrate(config, "./test/valid")
			_ = migrate.Reset()
		...
		Describe("NewPgMigrate", func() {
			It("should return Error when source folder is invalid", func() {
				_, err := NewPgMigrate(config, "./not/exist/folder")
			...
			It("should return Error when database cannot be connected", func() {
				...
				_, err := NewPgMigrate(&invalidConfig, "./test/valid")

				Expect(err).NotTo(BeNil())
				Expect(err.Error()).To(Equal("dial tcp 127.0.0.1:12345: connect: connection refused"))
			})
		})
		...
		Describe("PgMigrate", func() {
			It("should implement RDbMigrate interface", func() {
				migrate, _ := NewPgMigrate(config, "./test/valid")
				var _ appinterface.RDbMigrate = migrate
			})
		})
		...
		Describe("Up", func() {
			It("should return Error when migration up file is invalid", func() {
				...
				migrate, err := NewPgMigrate(config, "./test/invalid")
				Expect(err).To(BeNil())
			...
			It("should execute up migration", func() {
				...
				migrate, err := NewPgMigrate(config, "./test/valid")
				Expect(err).To(BeNil())
			...
			It("should execute up migration from current version to the latest migration", func() {
				...
				var migrate *PgMigrate
				migrate, err = NewPgMigrate(config, "./test/step-1")
				Expect(err).To(BeNil())
				...
				migrate, err = NewPgMigrate(config, "./test/step-2")
				Expect(err).To(BeNil())
		...
		Describe("Reset", func() {
			...
			It("should reset all the migrations in dirty state", func() {
				...
				migrate, err := NewPgMigrate(config, "./test/dirty-down")
				Expect(err).To(BeNil())
